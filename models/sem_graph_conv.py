...
class MutualSemGraphConv(nn.Module):
    ...
    def __init__(self, in_features, out_features, adj, adj_mutual, feature_mutual, bias=True):
        '''
        ...
        :param feature_mutual: temporarily it is (N-1)N * 34 matrix, such as
        [x11,y11,...,x172,y172
         x11,y11,...,x173,y173
        ...]
        :param bias:
        ...
        self.adj_mutual_W = nn.Parameter(torch.zeros(size=(self.adj.size(0)*2, self.adj.size(0)), dtype=torch.float))
    ...
    def forward(self, input):
        ...
        e = torch.matmul(input[1], self.adj_mutual_W)  ## N(N-1) * 17
        e = e.reshape([-1]) ##-->这样来
